...
export default function CajasReportePage() {
  ...
  const [rawRows, setRawRows] = useState([]); // filas crudas de la vista (1 fila = 1 línea)
  ...
  const [selectedCaja, setSelectedCaja] = useState(null); // código caja
  const [activeTab, setActiveTab] = useState("list"); // list | detail

  // Paginación listado de cajas
  const [listPage, setListPage] = useState(1);
  const [listPageSize, setListPageSize] = useState(10);

  // Paginación detalle de líneas
  const [detailPage, setDetailPage] = useState(1);
  const [detailPageSize, setDetailPageSize] = useState(20);

  // ===== Fetch base (usa la vista existente) =====
  ...
  // Agrupa por caja ⇒ { cabecera, lineas[] }
  const grouped = useMemo(() => {
    ...
    for (const r of rawRows) {
      ...
      map.get(key).lineas.push({
        ...
        TipoItem: r.TipoItem,
        LoteItem: r.LoteItem,
        CategoriaDetalleCaja: r.CategoriaDetalleCaja ?? r.U_LS_CATDET ?? "",
        CodigoItemUnico: r.CodigoItemUnico ?? r.U_LS_CODUNICO ?? "",
      });
    }
    return map;
  }, [rawRows]);

  // Filtrado por fecha + texto
  const filteredKeys = useMemo(() => {
    ...
    return Array.from(grouped.keys()).filter((k) => {
      ...
      const hay = [
        ...
        g.cab.Almacen,
        g.cab.ClaseCaja,
        ...g.lineas.map((l) => `${l.CodigoItem} ${l.Descripcion} ${l.LoteItem}`),
      ]
        .join(" ")
        .toLowerCase();
  ...
  // Reset de páginas cuando cambian filtros o selección
  useEffect(() => { setListPage(1); }, [fechaDesde, fechaHasta, textoBusqueda]);
  useEffect(() => { setDetailPage(1); }, [selectedCaja]);

  const totalCajas = filteredKeys.length;
  const totalLineas = useMemo(
    () => filteredKeys.reduce((acc, k) => acc + grouped.get(k).lineas.length, 0),
  ...
  function exportXLSX() {
    ...
    const name = selectedCaja && grouped.has(selectedCaja)
  ...
  async function exportPDF() {
    ...
    const name = keys.length === 1 ? `Caja_${keys[0]}_${hoy()}.pdf` : `Cajas_${hoy()}.pdf`;
    doc.save(name);
  }
    ...
    <div className={styles.topBar}>
      ...
      <div className={styles.filters}>
        ...
        <div className={styles.summary}>
          <span>{filteredKeys.length} cajas</span>
          <span>{filteredKeys.reduce((acc, k) => acc + grouped.get(k).lineas.length, 0)} líneas</span>
        </div>
      </div>

      <div className={styles.tabs}>
        <button className={activeTab === "list" ? styles.tabActive : styles.tab} onClick={() => setActiveTab("list")}>Listado</button>
        <button
        ...
        <div className={styles.card}>
          <table className={styles.table}>
            ...
            <tbody>
              {paginatedKeys.map((k) => {
                const g = grouped.get(k);
                return (
                  <tr key={k} className={selectedCaja === k ? styles.trSelected : undefined}>
                    ...
                    <td>
                      <button className={styles.btnSm} onClick={() => { setSelectedCaja(k); setActiveTab("detail"); }}>
                        Ver
                      </button>
                    </td>
                  </tr>
          ...
          <div className={styles.pagination}>
            ...
            <div className={styles.paginationRight}>
              ...
              <button className={styles.btnSm} disabled={listPage===1} onClick={()=>setListPage(p=>Math.max(1,p-1))}>Anterior</button>
              <button className={styles.btnSm} disabled={listPage===listTotalPages} onClick={()=>setListPage(p=>Math.min(listTotalPages,p+1))}>Siguiente</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className={styles.card}>
            ...
            <table className={styles.table}>
              ...
              <tbody>
                ...
                {paginatedLines.length === 0 && (
                  <tr><td colSpan={8} className={styles.empty}>Sin líneas</td></tr>
                )}
              </tbody>
            </table>
            ...
            <div className={styles.pagination}>
              ...
              <div className={styles.paginationRight}>
                ...
                <button className={styles.btnSm} disabled={detailPage===1} onClick={()=>setDetailPage(p=>Math.max(1,p-1))}>Anterior</button>
                <button className={styles.btnSm} disabled={detailPage===detailTotalPages} onClick={()=>setDetailPage(p=>Math.min(detailTotalPages,p+1))}>Siguiente</button>
              </div>
            </div>
          </div>
        </div>
...
// ================================
// app/cajas/CajasReporte.module.css
// ================================
